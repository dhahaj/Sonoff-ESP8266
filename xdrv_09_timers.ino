...
void TimerEverySecond()
{
  if (RtcTime.valid) {
    if (!RtcTime.hour && !RtcTime.minute && !RtcTime.second) {
      TimerSetRandomWindows();  // Midnight
    }
...
#endif  // USE_RULES
                if (devices_present) {
                  ExecuteCommandPower(xtimer.device + 1, xtimer.power, SRC_TIMER);
                }
            }
          }
        }
          ...
          if (!root.success()) {
            ...
            if (root[UpperCase_P(parm_uc, PSTR(D_JSON_TIMER_TIME))].success()) {
              ...
              if (substr != NULL) {
                ...
                value = atoi(substr);
                if (sign) {
                  value += 12;  // Allow entering timer offset from -11:59 to -00:01 converted to 12:01 to 23:59
                }
                if (value > 23) {
                  value = 23;
                }
