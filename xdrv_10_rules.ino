...
bool RulesRuleMatch(byte rule_set, String &event, String &rule)
{
  ...
  if (rules_teleperiod) {
    ...
    if (ppos == -1) {
      return false;  // No pre-amble in rule
    }
  ...
  if (!root[rule_task][rule_name].success()) {
    return false;
  }
  ...
  if (str_value) {
    ...
    switch (compare) {
      ...
      case '=':
        ...
        if (!strcasecmp(str_value, rule_svalue)) {
          match = true;  // Compare strings - this also works for hexadecimals
        }
        break;
      case ' ':
...
bool RulesProcessEvent(char *json_event)
{
  ...
  for (byte i = 0; i < MAX_RULE_SETS; i++) {
    if (strlen(Settings.rules[i]) && bitRead(Settings.rule_enabled, i)) {
      if (RuleSetProcess(i, event_saved)) {
        serviced = true;
      }
    }
  }
